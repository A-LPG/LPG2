...
| `rust` + `parser` | `rust_*_parser_run` / `rust_automatic_ast_*_behavior` | 确定性/回溯 parser；nested/list/`parent_saved`/preorder AST 行为断言 |
...
CI 的 `runtime-integration` job 会 clone `LPG-cpp-runtime`（含嵌套子模块）和
`LPG-rust-runtime`，并开启 `LPG2_REQUIRE_CPP_PARSER_TESTS` 与
...
| 区域 | 说明 |
|------|------|
| C / ML / Plx / Plxasm / Xml 后端 | **已弃用**桩实现：选择时发出 Warning；请改用完整后端 |
| `grammar/.lpg/` vs `src/` | 可能存在表漂移；以 `src/` 编译结果为准，晋升需走 BOOTSTRAP 流程 |
| Rust automatic AST | Generator-side closed loop for `nested`、list、`parent_saved`、`visitor=default` / `visitor=preorder`：由 `rust_automatic_ast_*_behavior` 断言 root downcast、list size/children、parent 指针与 preorder 顺序。复杂语法仍建议小步验证；不宣称与 Java/C++ 全量 AST 变体对等 |
