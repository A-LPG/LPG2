...
- Recover / prosthetic AST: **done for Java**, including `$allocation`. `%Recover Sym /. expr ./` embeds `expr` in `create(IToken error_token)` (may reference `error_token`); without a block a placeholder `AstToken` is used. `getProsthesisIndex` + `EmitProstheticAstFactories` + runtime `ProstheticAst`/`BacktrackingParser` close the loop (`java_automatic_ast_recover`). Still deferred for the other backends.
