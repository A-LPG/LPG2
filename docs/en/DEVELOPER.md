...
- Recover / prosthetic AST: **done for Java**. `Grammar::recovers` feeds `JavaTable` (`getProsthesisIndex(kind)` when `%Recover` symbols exist) and `JavaAction::EmitProstheticAstFactories` (a `ProstheticAst[] prostheticAst` field + `getProstheticAst()` override under `automatic_ast`). Runtime `lpg.runtime.ProstheticAst`, `RuleAction.getProstheticAst()` (default null), `ParseTable.getProsthesisIndex()` (default 0), and `BacktrackingParser` close the loop (synthesize a placeholder `AstToken` instead of throwing). Covered by `java_automatic_ast_recover`. Still deferred for the other backends, and user `$allocation` expression syntax is not yet parsed from the grammar (placeholder node is emitted)
