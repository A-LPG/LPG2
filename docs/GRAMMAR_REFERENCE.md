...
启用 `%Options ebnf` 或 CLI `-ebnf` 后，在 `%Rules` 中支持 Antlr/W3C 风格子集，并含 ISO 风格括号糖：
...
| `[X]` | 等价 `(X)?` |
| `{X}` | 等价 `(X)*` |
| `X*$Foo` / `X$Foo*` | 量词结果在父规则上的字段宏为 `$Foo`（`?`/`+` 同理）；list AST 仍按 operand 的 `$$X` 形状 |

规则：

- 可与普通 BNF 混写；顶层 `|` 仍是 LPG 的 alternate 规则分隔符，组内 `|`（含 `()` / `[]` / `{}`）属于 EBNF 选择。
- 运算符终结符须写成引号或别名：`'+'` / `PLUS`，不能写成裸 `+`。
- 组内允许 action block（`/. … ./`）；量词两侧不可同时写字段宏（`X$Foo*$Bar` 报错）。
...
- 可运行示例：[`examples/ebnf-call/`](../examples/ebnf-call/)。
...
- Rust 支持 `automatic_ast=toplevel`：AST 类写到 `<out_directory>/ast/*.rs`，parser 通过 `*_ast_includes.rs` `include!` 接入（未指定 `-ast_directory` 时默认 `<out>/ast`）
- 八后端均有 GLR v2 驱动（GSS/SPPF）；可运行示例：[examples/calculator](../examples/calculator/)（`automatic_ast=nested` + `dtParserTemplateF.gi`）
