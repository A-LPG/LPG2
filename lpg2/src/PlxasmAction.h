#ifndef PlxasmAction_INCLUDED
#define PlxasmAction_INCLUDED

#include "tuple.h"
#include "PlxAction.h"

class PlxasmAction : public PlxAction
{
public:

    PlxasmAction(Control *control_, Blocks *action_blocks_, Grammar *grammar_, MacroLookupTable *macro_table_)
                : PlxAction(control_, action_blocks_, grammar_, macro_table_)
    {
        if (option -> automatic_ast != Option::NONE)
        {
            control_ -> option -> EmitError(0, "Cannot automatically generate AST for programming language Plxasm");
            return_code = 12;
        }
    }
    virtual ~PlxasmAction() {}
