...
//
void CSharpAction::EmitAstClassCloser(TextBuffer &code_buf, ActionFileSymbol * /*top_level_file*/, bool /*list_extension_closer*/)
{
    if (option->IsNested())
    {
        code_buf.Put("    }\n\n");
    ...
    else
    {
        if (option->IsPackage())
        {
            // Close the enclosing namespace (indentation is empty at top level).
            code_buf.Put("}\n");
        }
        code_buf.Put("}\n\n");
    }
...
void CSharpAction::MaybeEmitAstRootInterface(ActionFileLookupTable &ast_filename_table,
