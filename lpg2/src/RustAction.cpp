...
namespace
{
    // Strip a trailing directory separator if present.
...
{
    // The parser/action file always parks post-template AST helpers (prosthetic
    // factories, toplevel include!) in NestAst so they land after %Trailers
    // closes `impl $action_type { ... }`. Satellite toplevel AST files use Body.
    if (option->IsTopLevel() &&
        ast_filename_symbol != option->DefaultBlock()->ActionfileSymbol())
    {
...
void RustAction::EmitTopLevelAstPrelude(TextBuffer *buffer) const
{
    // Satellite AST files are include!'d into the parser module, which already
    // provides `use lpg2::prelude::*` / Rc / RefCell. Do not re-emit imports
    // (E0252 duplicate imports). Also never copy template %Globals here.
    buffer->Put("// include!'d into parser module — imports come from the host file\n\n");
}

void RustAction::EnsureTopLevelAstDirectory()
{
    toplevel_ast_dir_prefix.clear();
    if (! option->IsTopLevel())
        return;

    // Empty -ast_directory yields prefix "./" (or package-relative "../…") and
    // can dump Root.rs into CWD. Prefer <out_directory>/ast/ next to the parser
    // unless the user passed an explicit absolute/out-of-tree -ast_directory.
    const char *prefix = option->ast_directory_prefix ? option->ast_directory_prefix : "";
    bool use_default =
        std::strcmp(prefix, "./") == 0 || std::strcmp(prefix, ".\\") == 0 ||
        std::strcmp(prefix, "../") == 0 || std::strcmp(prefix, "..\\") == 0 ||
        std::strlen(prefix) == 0;
    ...
    if (use_default)
    {
        std::string out = WithoutTrailingSlash(option->out_directory);
        if (out.empty())
            out = ".";
        ast_dir = out + "/ast";
    }
    ...
    {
        ast_dir = WithoutTrailingSlash(prefix);
    }

    std::error_code ec;
    std::filesystem::create_directories(ast_dir, ec);
    toplevel_ast_dir_prefix = ast_dir + "/";
}
                                       ...
                                       ActionFileSymbol *& /*out_container*/)
{
    ...
    toplevel_ast_relpaths.clear();
    EnsureTopLevelAstDirectory();
}

void RustAction::FinishAstEmitContext(ActionFileSymbol * /*container*/)
{
    if (! option->IsTopLevel() || toplevel_ast_relpaths.empty())
        return;

    // Emit a sidecar include list next to the parser; NestAst of the parser
    // pulls it in so rule actions see Root/Item/… in the same module (like nested).
    std::string includes_name = std::string(option->action_type) + "_ast_includes.rs";
    std::string out = WithoutTrailingSlash(option->out_directory);
    if (out.empty())
        out = ".";
    std::string includes_path = out + "/" + includes_name;

    FILE *fp = std::fopen(includes_path.c_str(), "wb");
    if (! fp)
    {
        option->EmitError(0, "Unable to write Rust toplevel AST include list");
        return;
    }
    std::fprintf(fp, "// Generated by LPG2 %s — include! list for automatic_ast=toplevel\n",
                 Control::VERSION);
    for (size_t i = 0; i < toplevel_ast_relpaths.size(); i++)
        std::fprintf(fp, "include!(%c%s%c);\n", '"', toplevel_ast_relpaths[i].c_str(), '"');
    std::fclose(fp);

    ActionFileSymbol *default_file = option->DefaultBlock()->ActionfileSymbol();
    TextBuffer *nest = default_file->BufferForNestAst();
    nest->Put("\n// automatic_ast=toplevel satellite types\n");
    nest->Put("include!(\"");
    nest->Put(includes_name.c_str());
    nest->Put("\");\n");
}
                                            ...
                                            bool)
{
    ...
    const char *dir_prefix = option->ast_directory_prefix ? option->ast_directory_prefix : "";
    if (option->IsTopLevel() && ! toplevel_ast_dir_prefix.empty())
        dir_prefix = toplevel_ast_dir_prefix.c_str();
    int filename_length = (int) strlen(dir_prefix) + (int) strlen(type_name) + (int) strlen(filetype);
    ...
    if (option->IsTopLevel())
    {
        EmitTopLevelAstPrelude(buffer);

        // Record path relative to out_directory for the include! sidecar.
        std::string abs = filename;
        std::string out = WithoutTrailingSlash(option->out_directory);
        std::string rel = abs;
        if (! out.empty())
        {
            std::string prefix = out + "/";
            if (abs.compare(0, prefix.size(), prefix) == 0)
                rel = abs.substr(prefix.size());
            else if (abs.compare(0, out.size(), out) == 0 &&
                     abs.size() > out.size() &&
                     (abs[out.size()] == '/' || abs[out.size()] == '\\'))
                rel = abs.substr(out.size() + 1);
        }
        // Prefer forward slashes in include! paths.
        for (size_t i = 0; i < rel.size(); i++)
            if (rel[i] == '\\')
                rel[i] = '/';
        toplevel_ast_relpaths.push_back(rel);
    }
    else if (*option->ast_package != '\0')
    {
        buffer->Put("// mod ");
                                                      ...
                                                      bool needs_environment)
{
    ...
    // For toplevel AST satellites, skip template %Globals (contains PrsStreamAdapter
    // and other parser-only items). Nested mode never uses this path for AST types.
    if (option->IsTopLevel())
        return file_symbol;
