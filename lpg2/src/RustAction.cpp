...
void RustAction::GenerateVisitorHeaders(TextBuffer &b, const char *indentation, const char *modifiers, const char* def_prefix)
{
    if (option -> visitor != Option::NONE)
    {
        ...
        {
            b.Put(header.c_str());
            b.Put("Accept(v Arc<dyn IAst>Visitor)");
            if (def_prefix){
                b.Put("{}");
            }
            b.Put("\n");
        ...
        if (option -> visitor & Option::DEFAULT)
        {
            ...
            b.Put(", o Box<dyn std::any::Any>) Box<dyn std::any::Any>");
            if (def_prefix) {
                b.Put("{ return None }");
            }
        }
                                        ...
                                        BitSet &, const char* def_prefix)
{
    if (option -> visitor & Option::DEFAULT)
    {
        ...
        b.Put(def_prefix); b.Put("      AcceptWithArg(v ");
                                     b.Put(visitorFactory->argument_visitor_type);
                                     b.Put(", o Box<dyn std::any::Any>){ v.Visit");b.Put(element.real_name); b.Put("WithArg(my, o) }\n");

        b.Put(def_prefix); b.Put("      AcceptWithResult(v ");
                                     b.Put(visitorFactory->result_visitor_type);
                                     b.Put(") Box<dyn std::any::Any>{return v.Visit");b.Put(element.real_name); b.Put("WithResult(my) }\n");

        b.Put(def_prefix); b.Put("      AcceptWithResultArgument(v ");
                                     b.Put(visitorFactory->result_argument_visitor_type);
                                     b.Put(", o Box<dyn std::any::Any>) Box<dyn std::any::Any>{return v.Visit"); b.Put(element.real_name); b.Put("WithResultArgument(my, o) }\n");
    }
    if (option -> visitor & Option::PREORDER)
    {
        b.Put("\n");
        b.Put(def_prefix); b.Put("       Accept(v Arc<dyn IAst>Visitor){\n");
        ...
        else
        {
            ...
            for (int i = 0; i < symbol_set.Size(); i++)
            {
                ...
                b.Put("if my._").Put(symbol_set[i] -> Name()).Put("{").Put("my._").Put(symbol_set[i] -> Name()).Put(".Accept(v)}\n");
            }
                                              ...
                                              ClassnameElement &element, const char* def_prefix)
{
    if (! element.is_terminal_class)
    {
        ...
        for (int i = 0; i < symbol_set.Size(); i++)
        {
        	...
        	b+"        if my._"+symbol_set[i]->Name()+"{  list.Add(my._"+symbol_set[i] -> Name()+") }\n";
                                                  ...
                                                  SymbolLookupTable &type_set)
{
    ...
    {
        Symbol *symbol = type_set[i];
    	b+"    Visit"+symbol->Name()+"WithArg(n  *"+symbol -> Name()+", o Box<dyn std::any::Any>) \n";
    }

	 b.Put("\n");

     b.Put("    VisitWithArg(n  Arc<dyn IAst>, o Box<dyn std::any::Any>) \n");
                                                ...
                                                SymbolLookupTable &type_set)
{
    ...
    {
        Symbol *symbol = type_set[i];
    	b+"    Visit"+symbol->Name()+"WithResult(n  *"+symbol -> Name()+") Box<dyn std::any::Any>\n";
    }

	b.Put("\n");
	b.Put("    VisitWithResult(n  Arc<dyn IAst>) Box<dyn std::any::Any>\n");
                                                        ...
                                                        SymbolLookupTable &type_set)
{
    ...
    {
        Symbol *symbol = type_set[i];
        b+"    Visit"+symbol->Name()+"WithResultArgument(n  *"+symbol -> Name()+", o Box<dyn std::any::Any>) Box<dyn std::any::Any>\n";
    }

	b.Put("\n");
	b.Put("    VisitWithResultArgument(n Arc<dyn IAst>, o Box<dyn std::any::Any>) Box<dyn std::any::Any>\n");
                                                  ...
                                                  SymbolLookupTable &type_set)
{
    ...
    assert(option -> visitor & Option::PREORDER);
     ...
     b.Put("  Arc<dyn IAst>Visitor\n");

     b.Put("    Visit");
                                 b.Put("(n  Arc<dyn IAst>");
                                                      ...
                                                      SymbolLookupTable &type_set)
{
    ...
    std::string def_prefix_holder = "impl ";
    ...
    for (int i = 0; i < type_set.Size(); i++)
    {
        ...
        b.Put(def_prefix);
        b+"      Visit"+symbol->Name()+"WithArg(n *"+ symbol->Name()+ ", o Box<dyn std::any::Any>)  { my.UnimplementedVisitor(\"Visit"+ symbol->Name()+ "WithArg(" + symbol->Name() + ", Box<dyn std::any::Any>)\") }\n";
        b.Put("\n");
    }
    ...
    b.Put(def_prefix);
    b + "     VisitWithArg(n Arc<dyn IAst>, o Box<dyn std::any::Any>){\n";
    b + "     switch n2 := n.(type) {\n";
    for (int i = 0; i < type_set.Size(); i++)
    {
        Symbol* symbol = type_set[i];
                                                    ...
                                                    SymbolLookupTable &type_set)
{
    ...
    b.Put(def_prefix); b.Put("       UnimplementedVisitor(s  string) Box<dyn std::any::Any>{ return None }\n\n");
    {
        for (int i = 0; i < type_set.Size(); i++)
        {
            Symbol *symbol = type_set[i];
            ...
            b.Put(def_prefix);
        	b+"     Visit"+symbol->Name()+"WithResult(n *"+ symbol->Name()+") Box<dyn std::any::Any>{ return  my.UnimplementedVisitor(\"Visit" +
                symbol->Name()+ "WithResult(*" + symbol -> Name() + ")\")}\n\n";

            b.Put(def_prefix);
            b + "     Visit" + symbol->Name() + "WithResultArgument(n *" + symbol->Name()
        	+ ", o Box<dyn std::any::Any>) Box<dyn std::any::Any>{ return  my.UnimplementedVisitor(\"Visit" +
                symbol->Name() + "WithResultArgument(*" + symbol->Name() + ", Box<dyn std::any::Any>)\")}\n\n";
    ...
    b.Put(def_prefix) + "     VisitWithResult(n Arc<dyn IAst>) Box<dyn std::any::Any>{\n";
    ...
    b.Put("        default:{ return None}\n");
    b.Put("     }\n}\n");



	b.Put(def_prefix)+"     VisitWithResultArgument(n Arc<dyn IAst>, o Box<dyn std::any::Any>) Box<dyn std::any::Any>{\n";
    b + "     switch n2 := n.(type) {\n";
    for (int i = 0; i < type_set.Size(); i++)
    {
        Symbol* symbol = type_set[i];
                                                      ...
                                                      SymbolLookupTable &type_set)
{
    ...
    b.Put(def_prefix); b.Put("     PreVisit(element Arc<dyn IAst>) bool{ return true }\n\n");
    b.Put(def_prefix); b.Put("     PostVisit(element  Arc<dyn IAst>) {}\n\n");
    {
        for (int i = 0; i < type_set.Size(); i++)
        {
            Symbol *symbol = type_set[i];
	...
	b.Put("\n");
    b.Put(def_prefix);
	b.Put("     Visit(n Arc<dyn IAst>) bool{\n");
	b + "     switch n2 := n.(type) {\n";
	for (int i = 0; i < type_set.Size(); i++)
	{
	 Symbol* symbol = type_set[i];
                                 ...
                                 const char *classname)
{
    ...
    {
	       b.Put("     parent Arc<dyn IAst>\n");
    }
    b.Put("}\n");

     b.Put("func New").Put(classname).Put("2(left_i_token  Rc<dyn IToken> , right_i_token  Rc<dyn IToken> ) *").Put(classname).Put("{\n");
     b.Put("        my := new(").Put(classname).Put(")\n");
     b.Put("        my.left_i_token = left_i_token\n");
     b.Put("        my.right_i_token = right_i_token\n");
     b.Put("        return my\n");
     b.Put("}\n\n");

     b.Put("func New").Put(classname).Put("(token  Rc<dyn IToken>) *").Put(classname).Put("{\n");
     b.Put("        my := new(").Put(classname).Put(")\n");
     b.Put("        my.left_i_token = token\n");
     b.Put("        my.right_i_token = token\n");
     b.Put("        return my\n");
     b.Put("}\n\n");
    ...
    if (option -> glr)
    {
         b.Put(def_prefix); b.Put("    GetNextAst() Arc<dyn IAst>  { return my.next_ast }\n");
         b.Put(def_prefix); b.Put("    SetNextAst(n  Arc<dyn IAst>) { my.next_ast = n }\n");
         b.Put(def_prefix); b.Put("    ResetNextAst()  { my.next_ast = nil }\n");
    }
    else
    {
	     b.Put(def_prefix); b.Put("     GetNextAst() Arc<dyn IAst>  { return None }\n");
    }
    ...
    if (option -> parent_saved)
    {
         ...
         b.Put(def_prefix); b.Put("      SetParent(parent Arc<dyn IAst> )  { my.parent = parent }\n");
         b.Put(def_prefix); b.Put("      GetParent() Arc<dyn IAst> { return my.parent }\n");\
    }
    else
    {
    ...
    b.Put("\n");
     b.Put(def_prefix); b.Put("     GetLeftIToken()  Rc<dyn IToken> { return my.left_i_token }\n");
     b.Put(def_prefix); b.Put("     GetRightIToken()  Rc<dyn IToken> { return my.right_i_token }\n");
     b.Put(def_prefix); b.Put("     GetPrecedingAdjuncts()  Vec<Rc<dyn Rc<dyn IToken>>> { return my.left_i_token.GetPrecedingAdjuncts() }\n");
     b.Put(def_prefix); b.Put("     GetFollowingAdjuncts()  Vec<Rc<dyn Rc<dyn IToken>>> { return my.right_i_token.GetFollowingAdjuncts() }\n\n");

     b.Put(def_prefix); b.Put("      ToString()string{\n");
     b.Put("        return my.left_i_token.GetILexStream().ToString(my.left_i_token.GetStartOffset(), my.right_i_token.GetEndOffset())\n");
     b.Put("}\n\n");
    ...
    if (option -> parent_saved)
    {
         ...
         b.Put("     * A list of all children of my node, don't including the null ones.\n");
         b.Put("     */\n");
         b.Put(def_prefix); b.Put("    GetAllChildren() &ArrayList{ return None }\n");
    }
    else
    {
         b.Put(def_prefix); b.Put("      GetChildren() &ArrayList{\n");
    ...
    //
    // Not Preorder visitor? generate dummy accept method to satisfy Arc<dyn IAst> abstract declaration of Accept(IAstVisitor);
                                             ...
                                             const char *classname)
{
	...
	b+"pub trait "+ abstract_ast_list_classname + " {\n";
    b + "     *" + option->ast_type + "\n";
    ...
    // generate constructors for list class
     b + "func New" + abstract_ast_list_classname + 
        "(leftToken  Rc<dyn IToken>, rightToken  Rc<dyn IToken>, leftRecursive bool)*" + abstract_ast_list_classname +"{\n";
    ...
    const auto def_prefix = def_prefix_holder.c_str();
     ...
     b.Put(def_prefix); b.Put("      GetElementAt(i int ) Arc<dyn IAst>{\n");
     b + "    var k int\n";
     ...
     b.Put(def_prefix); b.Put("      Add(element Arc<dyn IAst>) bool {\n");
     b.Put("        my.AddElement(element)\n");
     b.Put("        return true\n");
     b.Put("}\n\n");

     b.Put(def_prefix);
						b.Put("      AddElement(element Arc<dyn IAst>){\n");

     b.Put("        my.list.Add(element)\n");
     b.Put("        if my.leftRecursive{\n");
     b.Put("             my.right_i_token = element.GetRightIToken()\n");
     b.Put("        }else{\n");
     b.Put("          my.left_i_token = element.GetLeftIToken()\n");
     b.Put("        }\n");
     b.Put("}\n\n");
                                      ...
                                      const char *classname)
{
     ...
     */
     ...
     b + "func New" + classname + "(token  Rc<dyn IToken>)*" + classname+"{\n";
     b + "      my := new(" + classname + ")\n";
    ...
    b.Put(def_prefix); b.Put("      GetIToken()  Rc<dyn IToken>{ return my.left_i_token }\n");
    b.Put(def_prefix); b.Put("      ToString()  string  { return my.left_i_token.ToString() }\n\n");
    ...
    const char* super_prefix, const char* def_prefix)
{
    ...
    //
    b.Put(def_prefix);
	b + "     " + " AddElement(_" + element_name + " Arc<dyn IAst>){ \n";
    ...
    if (option -> visitor & Option::DEFAULT)
    {
        ...
        b.Put(def_prefix);
    	b+"      AcceptWithArg(v  "+visitorFactory-> argument_visitor_type + ", o Box<dyn std::any::Any>){\n";
        ...
        //
        b.Put(def_prefix);
    	b+"      AcceptWithResult(v "+visitorFactory-> result_visitor_type + ") Box<dyn std::any::Any>{\n";
        b + "      " + "var i=0\n";
        ...
        b.Put(def_prefix);
    	b+"      AcceptWithResultArgument(v "+visitorFactory-> result_argument_visitor_type + ", o Box<dyn std::any::Any>) Box<dyn std::any::Any>{\n";
        b + "      " + "var i=0\n";
        b + "       var result = NewArrayList()\n";
        b + "      " + "for i=0; i < my.Size(); i++{\n";
        if (ctc.FindUniqueTypeFor(element.array_element_type_symbol->SymbolIndex()) != NULL)
                                   ...
                                   Array<const char *> &)
{
	...
	b + "func New" + classname + "(leftToken  Rc<dyn IToken>, rightToken  Rc<dyn IToken> , leftRecursive bool)*" + classname + "{\n";
	 b + "      my := new(" + classname + ")\n";
...

{
    ...
    const auto def_prefix = def_prefix_holder.c_str();
     ...
     b + "pub trait " + special_array.name + " {\n";
     b + "    *" + classname + "\n";
	...
	b +  "func New" + special_array.name + 
        "(environment *" +option->action_type + ", left_i_token  Rc<dyn IToken>,  right_i_token  Rc<dyn IToken>, leftRecursive bool)*"+ special_array.name+"{\n";

     b + "      my := new(" + special_array.name + ")\n";
     b + "      my." + classname + " = New" + classname + "(left_i_token, right_i_token, leftRecursive)\n";
                                   ...
                                   Array<const char *> &)
{
    ...
    if (element.is_terminal_class)
    {
        ...
        if (symbol_set.Size() == 1) // if the right-hand side contains a symbol ...
        {
            b.Put(def_prefix); b.Put("      Get");
                                         ...
                                         b.Put("()IToken{ return my.left_i_token; }\n\n");
        }

         b + "func New" + classname + "(";
        if (element.needs_environment)
        {
            b+"environment *"+ option->action_type + ",token Rc<dyn IToken> )*"+ classname+"{\n";
             b + "      my := new(" + classname + ")\n";
             b + "      my.environment = environment;\n";
        }
        else
        {
            b + "token Rc<dyn IToken> )*" + classname + "{\n";
             b + "      my := new(" + classname + ")\n";
        }
    ...
    else 
    {
        ...
        b.Put("*").Put(classname).Put("{\n");
         ...
         b + "      my := new(" + classname + ")\n";
         b + "      my." + option->ast_type + " = New" + option->ast_type + "2(left_i_token, right_i_token)\n";
        if (element.needs_environment)
        {
        ...
        for (int i = 0; i < symbol_set.Size(); i++)
        {
            ...
            if (option -> parent_saved)
            {
                ...
                b+"if _" + symbol_set[i]->Name()+"{\n";
                b.Put("        var trait_ Box<dyn std::any::Any> = _").Put(symbol_set[i]->Name()).Put("\n");
            	b.Put("        ");
                                             ...
                                             Array<const char *> &)
{
    ...
    if (symbol_set.Size() == 1) // if the right-hand side contains a symbol ...
    {
        b.Put(def_prefix); b.Put("      Get");
                                     b.Put(symbol_set[0] -> Name());
                                     b.Put("() Rc<dyn IToken>{ return my.left_i_token; }\n\n");
    }
    ...
    const char*)
{
	...
	b+"pub trait "+ astRootInterfaceName.c_str() + " {\n";
     b.Put("     GetLeftIToken()  Rc<dyn IToken>\n");
     b.Put("     GetRightIToken()  Rc<dyn IToken>\n");

    GenerateVisitorHeaders(b, "", "", nullptr);
	b.Put("}\n\n");

    b + "func " + castToAny.c_str() + "(i Box<dyn std::any::Any>) Box<dyn std::any::Any>{return i}";
    b.Put("\n\n");
    //castToAny
                ...
                else
                {
                    ...
                    if (grammar->IsTerminal(symbol))
                    {
                        ...
                        }else
                        {
                            ...
                            if (strcmp(actual_type, grammar->Get_ast_token_classname()) != 0)
                            {
                                std::string anyCastToHelper = "any_cast_to_";
