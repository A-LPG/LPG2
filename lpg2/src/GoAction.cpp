...
//
TextBuffer *GoAction::AstCodeBuffer(ActionFileSymbol *file)
{
    return GetBuffer(file);
}

void GoAction::EmitAstClassCloser(TextBuffer &code_buf, ActionFileSymbol * /*top_level_file*/, bool list_extension_closer)
{
    // No trailing "}" for Go; list-extension and rule closers differ only in indent.
    code_buf.Put(list_extension_closer ? "\n\n" : "    \n\n");
}

void GoAction::MaybeEmitAstRootInterface(ActionFileLookupTable &ast_filename_table,
    ...
    Array<const char*>&, int rule_no)
{
    ...
    GenerateTerminalGcDeleteReminder(b, space, rule_no, allocation_element, "GetRhsSym(1)");
