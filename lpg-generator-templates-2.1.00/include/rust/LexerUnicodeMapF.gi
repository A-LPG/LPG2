...
/. 
    ...
    fn $super_stream_class$_init_token_kind() -> Vec<i32> {
        let mut token_kind = vec![0i32; 0x10000];
        ...
        let mut i = 0x007Fi32;
        while i < 0xFFFF {
            if token_kind[i as usize] == 0 {
                token_kind[i as usize] = $sym_type::$prefix$UNUSED$suffix$;
            }
            i += 1;
        }
        token_kind
    ...
    fn $super_stream_class$_token_kind() -> &'static [i32] {
        use std::sync::OnceLock;
        static KIND: OnceLock<Vec<i32>> = OnceLock::new();
        KIND.get_or_init($super_stream_class$_init_token_kind).as_slice()
    }
