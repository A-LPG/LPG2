...
/. 
    ...
    func  New$super_stream_class( fileName string, inputChars []rune, tab int) (*$super_stream_class,error){
    ...
    var $super_stream_class$tokenKind =[]int{
    ...
    func (my *$super_stream_class) GetKind(i int) int {  // Classify character at ith location
            ...
            if c < 128 {//ASCII Character
                return $super_stream_class$tokenKind[c]
            }else{
                if c == 0xffff{
                    return $sym_type.$prefix$EOF$suffix$
                }else{
                    return $sym_type.$prefix$AfterASCII$suffix$
                }
            }
