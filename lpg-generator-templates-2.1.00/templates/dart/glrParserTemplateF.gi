--
-- Dart GLR parser driver template (GSS/SPPF runtime).
--
-- B E G I N N I N G   O F   T E M P L A T E   glrParserTemplateF
--
%Options programming_Language=dart,margin=4,glr
...
%Define
    ...
    /.
        ...
        %ast_class?  parse%entry_name([Monitor? monitor, int error_repair_count= 0]) 
        {
            ...
            try
            {
                return glrParser.parseEntry(%sym_type.%entry_marker, error_repair_count) as %ast_class?;
            }
...
%Headers
    /.
    class %action_type extends %super_class with ProstheticAstProvider implements RuleAction, GlrRecoverAction%additional_interfaces
    {
        ...
        late GLRParser glrParser;
        GLRParser getParser(){ return glrParser; }

        // During GLR-to-BT recovery, generated accessors must use BT stacks.
        BacktrackingParser? recoverParser;
        @override
        void setRecoverParser(BacktrackingParser? parser) {
            recoverParser = parser;
        }
        @override
        BacktrackingParser? getRecoverParser() { return recoverParser; }

        void setResult(Object? object1){
            if (recoverParser != null) recoverParser!.setSym1(object1);
            else glrParser.setSym1(object1);
        }
        ...
        %action_type([ILexStream? lexStream])
        {
            ...
            try
            {
                glrParser = GLRParser(null, %action_type.prsTable, this);
            }
            on  NotGLRParseTableException
            {
                throw (NotGLRParseTableException
                    ("Regenerate %prs_type.dart with -GLR option"));
            }
        ...
        %ast_class? parser([int error_repair_count = 0 ,Monitor?  monitor])
        {
            ...
            try{
                return glrParser.parse(error_repair_count) as %ast_class?;
            }
            on BadParseException catch(e){
                prsStream.reset(e.error_token); // point to error token
                var diagnoseParser = DiagnoseParser(prsStream, %action_type.prsTable);
