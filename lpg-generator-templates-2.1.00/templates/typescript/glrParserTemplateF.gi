...
--
%Options programming_Language=typescript,margin=4,glr
...
%Define
    ...
    /.
        ...
        public  parse%entry_name(monitor? : Monitor, error_repair_count : number = 0) : %ast_class | null
        {
            ...
            try
            {
                return <%ast_class> this.glrParser.parseEntry(%sym_type.%entry_marker, error_repair_count);
            }
            catch (ex)
            {
...
%Headers
    ...
    export class %action_type extends %super_class implements RuleAction%additional_interfaces
    {
        ...
        constructor(lexStream? :ILexStream)
        {
            ...
            try
            {
                this.glrParser = new GLRParser(null, %action_type.prsTable, <RuleAction> this);
            }
            catch (e)
            {
                if(e instanceof NotGLRParseTableException)
                throw (new NotGLRParseTableException
                                    ("Regenerate %prs_type.ts with -GLR option"));
        ...
        public parser(error_repair_count : number = 0 ,  monitor? : Monitor) :  %ast_class | null
        {
            ...
            try
            {
                return <%ast_class> this.glrParser.parse(error_repair_count);
            }
            catch (ex)
            {
               if( ex instanceof BadParseException ){
