...
5. Need all legal parse trees packed together → enable `-glr`, use `glrParserTemplateF.gi`, and Java or C++ runtime `GLRParser` (same-grammar-symbol/same-token-span `getNextAst()` forest; pure AST-building actions). `parser(N)` with `N>0` falls back to BT `%Recover` after GLR failure (single tree). No cyclic/epsilon-loop grammars; non-cyclic nullable rules work; other runtimes scaffolding only.
